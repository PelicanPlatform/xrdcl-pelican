...
TEST(HeaderParser, ParseInt) {
    {
        auto entry = "1234; ";
        auto [remainder, result, ok] = XrdClCurl::HeaderParser::ParseInt(entry);
        EXPECT_EQ(remainder, "; ");
...
TEST(HeaderParser, ParseString) {
    {
        auto entry = R"("foo")";
        auto [remainder, result, ok] = XrdClCurl::HeaderParser::ParseString(entry);
        EXPECT_EQ(ok, true);
        EXPECT_EQ(remainder, "");
...
TEST(HeaderParser, LinkEntry) {
    {
        ...
        auto [remainder, hdr, ok] = XrdClCurl::HeaderParser::LinkEntry::IterFromHeaderValue(entry);
        EXPECT_EQ(ok, true);
    ...
    {
        auto entry = R"(<https://example.com:8443/user/ligo/foo.txt>; rel="duplicate"; pri=15; depth=42)";
        auto [hdrList, ok] = XrdClCurl::HeaderParser::LinkEntry::FromHeaderValue(entry);
        EXPECT_EQ(ok, true);
